...
import optimizer.RotationFaker;
...
public enum RotationUtils
{
	...
	{
		RotationFaker rotationFaker = Client.CWHACK.getRotationFaker();
	...
	public static void setRotation(optimizer.Rotation rotation)
		{
			if (!rotation.isIgnoreYaw()) MC.player.setYaw(rotation.getYaw());
			if (!rotation.isIgnorePitch()) MC.player.setPitch(rotation.getPitch());
		}

	public static optimizer.Rotation getNeededRotations(Rotation pos) {
		return null;
	}
